  ...
  if (pVnode != NULL) {
    switch (qtype) {
      ...
      case SYNC_QUEUE:
        ...
        break;
      case SYNC_CTRL_QUEUE:
        size = taosQueueItemSize(pVnode->pSyncCtrlQ);
        break;
      case APPLY_QUEUE:
...
int32_t vmAllocQueue(SVnodeMgmt *pMgmt, SVnodeObj *pVnode) {
  ...
  if (pVnode->pWriteQ == NULL || pVnode->pSyncQ == NULL || pVnode->pSyncCtrlQ == NULL || pVnode->pApplyQ == NULL ||
      pVnode->pQueryQ == NULL || pVnode->pStreamQ == NULL || pVnode->pFetchQ == NULL) {
  ...
  dDebug("vgId:%d, sync-ctrl-queue:%p is alloced", pVnode->vgId, pVnode->pSyncCtrlQ);
