...
int32_t mndReadFile(const char *path, SMnodeOpt *pOption) {
  ...
  if (len <= 0) {
    mError("failed to read %s since content is null", file);
    goto _OVER;
  }
  ...
  if (root == NULL) {
    mError("failed to read %s since invalid json format", file);
    goto _OVER;
  }

  cJSON *deployed = cJSON_GetObjectItem(root, "deployed");
  if (!deployed || deployed->type != cJSON_Number) {
    mError("failed to read %s since deployed not found", file);
    goto _OVER;
  }
  ...
  if (!selfIndex || selfIndex->type != cJSON_Number) {
    mError("failed to read %s since selfIndex not found", file);
    goto _OVER;
  }
  pOption->selfIndex = selfIndex->valueint;

  cJSON *replicas = cJSON_GetObjectItem(root, "replicas");
  if (!replicas || replicas->type != cJSON_Array) {
    mError("failed to read %s since replicas not found", file);
    goto _OVER;
  }

  int32_t numOfReplicas = cJSON_GetArraySize(replicas);
  if (numOfReplicas <= 0) {
    mError("failed to read %s since numOfReplicas:%d invalid", file, numOfReplicas);
    goto _OVER;
  }
...
_OVER:
  ...
  if (code == 0) {
    mDebug("succcessed to read file %s, deployed:%d", file, pOption->deploy);
  }
...
int32_t mndWriteFile(const char *path, const SMnodeOpt *pOption) {
  char file[PATH_MAX] = {0};
  ...
  if (taosRenameFile(file, realfile) != 0) {
    terrno = TAOS_SYSTEM_ERROR(errno);
    mError("failed to rename %s since %s", file, terrstr());
    return -1;
  }

  mDebug("successed to write %s, deployed:%d", realfile, pOption->deploy);
  return 0;
}
