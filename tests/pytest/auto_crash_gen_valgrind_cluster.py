...
def get_auto_mix_cmds(args_list ,valgrind=valgrind_mode):
    ...
    if valgrind :
        ...
        crash_gen_cmd = 'cd %s && ./crash_gen.sh  --valgrind   -i 3 %s -g 0x32c,0x32d,0x3d3,0x18,0x2501,0x369,0x388,0x061a,0x2550,0x0707,0x707 '%(crash_gen_path ,arguments)

    else:

        crash_gen_cmd = 'cd %s && ./crash_gen.sh  -i 3 %s  -g 0x32c,0x32d,0x3d3,0x18,0x2501,0x369,0x388,0x061a,0x2550,0x0014,0x0707,0x707'%(crash_gen_path ,arguments)
...
def start_taosd():
    ...
    start_cmd = 'cd %s && python3 test.py -N 6 -M 3 '%(start_path)
